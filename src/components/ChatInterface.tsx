  ...
  return (
    <div className="flex flex-col h-full relative bg-background overflow-hidden">
      ...
      {isLoadingConversation ? (
        <div className={`flex-1 overflow-y-auto overflow-x-hidden pb-[240px] py-6 ${(selectedMode === 'text' && models?.text) || (selectedMode === 'image' && models?.image) ? 'pt-[calc(var(--model-rail-offset,0px)+12px)] scroll-pt-[calc(var(--model-rail-offset,0px)+12px)]' : ''}`}>
          <div className="w-full px-4 sm:px-6 lg:px-8">
            <MessageSkeleton />
          </div>
        </div>
      ) : messages.length > 0 ? (
        <div className={`flex-1 overflow-y-auto overflow-x-hidden pb-[240px] py-4 sm:py-6 ${(selectedMode === 'text' && models?.text) || (selectedMode === 'image' && models?.image) ? 'pt-[calc(var(--model-rail-offset,0px)+12px)] scroll-pt-[calc(var(--model-rail-offset,0px)+12px)]' : ''}`}>
