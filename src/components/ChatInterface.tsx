...
export function ChatInterface() {
  ...
  return (
    <div className="flex flex-col h-full relative bg-background overflow-hidden">
      ...
      <div className="flex-shrink-0 p-4 pb-6 bg-background border-t border-border/20">
        <div key={selectedMode} className="max-w-3xl mx-auto animate-scale-in">
          <form onSubmit={handleSubmit}>
            ...
            {attachedFiles.length > 0 && (
              <div className="mb-3 p-3 bg-card rounded-2xl border border-border/50">
            ...
            <div className="bg-card rounded-2xl border border-border/50 shadow-sm overflow-hidden">
              ...
              <div className="flex items-center h-[56px] px-3 gap-2">
                ...
                <button 
                  ...
                  className="flex-shrink-0 w-9 h-9 rounded-full bg-muted/60 flex items-center justify-center hover:bg-muted transition-colors"
                >
                  <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
                    <path d="M10 4V16M4 10H16" stroke="currentColor" className="text-muted-foreground" strokeWidth="2" strokeLinecap="round" />
                  </svg>
                </button>
                ...
                <input 
                  ...
                  placeholder="Message Zebvo AI..." 
                  disabled={isLoading} 
                  className="flex-1 bg-transparent outline-none text-[15px] placeholder:text-muted-foreground/60 disabled:opacity-50 text-foreground" 
                  maxLength={4000} 
                />

                {/* Character count */}
                {input.length > 3500 && (
                  <span className={`text-xs ${input.length > 3900 ? 'text-destructive' : 'text-muted-foreground'}`}>
                    {4000 - input.length}
                  </span>
                )}
